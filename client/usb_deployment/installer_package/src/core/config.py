...
class Config:
    ...
    DEFAULT_SERVER_URL = os.getenv('TENJO_SERVER_URL', "https://tenjo.adilabs.id")  # Production domain
    LEGACY_SERVER_URLS = [
        "http://103.129.149.67",
    ...
    def init_directories(cls):
        ...
        # Refresh server configuration (loads override when present)
    ...
    def refresh_server_settings(cls, server_url=None, persist=False):
        candidate = server_url or cls._load_server_override() or cls.DEFAULT_SERVER_URL
        sanitized = cls._sanitize_server_url(candidate)
