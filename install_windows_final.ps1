# Tenjo Windows PowerShell Installer - Final Bulletproof Version
...
Write-Host "   PowerShell Installer v3.0 - Final" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
function Test-InternetConnection {
    try {
        ...
        try {
            $null = Invoke-WebRequest -Uri "http://google.com" -Method Head -TimeoutSec 10 -UseBasicParsing
            return $true
        } catch {
            return $false
        }
...
function Download-File {
    ...
    try {
        ...
        # Use BITS transfer if available, otherwise use Invoke-WebRequest
...
try {
    ...
    Write-Host "[0/6] Testing internet connectivity..." -ForegroundColor Cyan
    ...
    try {
        ...
        # Python not found, continue to installation
    }
    ...
    if (-not $pythonFound) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Python installation completed but python command not found. Please restart PowerShell and try again."
        }
    ...
    try {
        ...
        # Git not found, continue to installation
    }
    ...
    if (-not $gitFound) {
        ...
        $gitVersion = & git --version 2>&1
        if ($LASTEXITCODE -ne 0) {
            throw "Git installation completed but git command not found. Please restart PowerShell and try again."
        }
    ...
    Write-Host ""
    Write-Host "[3/6] Preparing installation directory..." -ForegroundColor Cyan
    ...
    # Clone repository
    Write-Host ""
    Write-Host "[4/6] Downloading Tenjo from GitHub..." -ForegroundColor Cyan
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[WARNING] Some packages failed to install. Output:" -ForegroundColor Yellow
    ...
    # Test installation
    Write-Host ""
    Write-Host "[6/6] Testing installation and configuring startup..." -ForegroundColor Cyan
    ...
    # Test Tenjo config
    $configTest = & python -c "from src.core.config import Config; print('[TEST] Tenjo config loaded successfully')" 2>&1
    if ($LASTEXITCODE -ne 0) {
        Write-Host "[WARNING] Config import test failed, but files exist - may work at runtime" -ForegroundColor Yellow
        if (-not (Test-Path "src\core\config.py")) {
            throw "Tenjo source files not found properly"
        }
    } else {
    ...
    @"
@echo off
REM Auto-generated Tenjo startup script
...
"@ | Out-File -FilePath $uninstaller -Encoding ASCII
    ...
    Write-Host "============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "[✓] Python verified and working" -ForegroundColor Green
    Write-Host "[✓] Git verified and working" -ForegroundColor Green
    Write-Host "[✓] Tenjo installed to: $InstallDir" -ForegroundColor Green
